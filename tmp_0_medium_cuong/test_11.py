...
def test_ABC():
    ...
    class MetaclassTest(ABC):
        pass
    
    # Test metaclass inheritance
    class CustomMetaclass(abc.ABCMeta):
        pass
    
    class CustomABC(metaclass=CustomMetaclass):
        pass
    
    try:
        class InvalidCombination(MetaclassTest, CustomABC):
            pass
        assert False, "Should not allow conflicting metaclasses"
    except TypeError as e:
        assert "metaclass conflict" in str(e).lower()
    
    assert type(MetaclassTest) is abc.ABCMeta
    assert MetaclassTest.__metaclass__ == abc.ABCMeta
    assert MetaclassTest.__slots__ == ()
test_ABC()